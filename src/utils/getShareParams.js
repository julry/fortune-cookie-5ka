...
export const getShareParams = (result) => {
    ...
    const shareTitle =  isMobile ? `${result.title} - моя будущая профессия в компании «Пятёрочка». Хочешь узнать, какая карьера ждёт тебя в топовой компании в сфере ритейла? Переходи по ссылке и получи предсказание.` : 'Кто ты в офисе «Пятёрочки»?';
